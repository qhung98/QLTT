...
public class EditNews extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final News news = (News)intent.getSerializableExtra("editNews");
        ...
        edHeader.setText(news.getHeader());
        edContent.setText(news.getContent());
        edLink.setText(news.getLink());
        ...
        for(int i=0; i<listName.size(); i++){
            if(listName.get(i).equals(category))
        ...
        btnAddNews.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                News news = new News(id, edHeader.getText().toString(), image, edContent.getText().toString(),
                        edLink.getText().toString(), time, views, category);

                db.editNews(news);
                Toast.makeText(getBaseContext(), "SUA TIN TUC THANH CONG", Toast.LENGTH_SHORT).show();
